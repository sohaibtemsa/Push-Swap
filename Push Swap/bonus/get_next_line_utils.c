...
char	*ft_strjoin_g(char *storage, char *buffer)
{
	...
	str = malloc(sizeof(char) * ((ft_stln_g(storage) + ft_stln_g(buffer)) + 1));
	if (str == NULL)
		return (free(storage), NULL);
	if (storage)
		while (storage[++i] != '\0')
	...
	while (buffer[j] != '\0')
		str[i++] = buffer[j++];
	str[ft_stln_g(storage) + ft_stln_g(buffer)] = '\0';
	free(storage);
	return (str);
}
...
char	*get_rest(char *storage)
{
	...
	str = (char *)malloc(sizeof(char) * (ft_stln_g(storage) - i + 1));
	if (!str)
		return (NULL);
	i++;
	j = 0;
